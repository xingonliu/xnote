...
class ModelProfileStoreTest {
    @Test fun credentialsAreEncryptedExcludedFromProfileAndDeletedWithProfile() = runTest {
        ...
        try {
            ...
            store.recordCapabilities(saved, ModelCapabilities(textStreaming = true, tools = true))
            val reconfigured = store.save(store.list().single().copy(providerId = "remote-provider", providerName = "远端厂商"), null)
            assertFalse(reconfigured.capabilities.textStreaming)
            assertEquals("remote-provider", store.list().single().providerId)
            assertEquals(secret, store.credential(reconfigured))

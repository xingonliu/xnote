...
编辑页 Header 的返回、笔记本归属、撤销、重做和更多均为 Liquid Glass 按钮。底部左侧是独立圆形插入按钮，右侧默认展开排版、待办、引用和收起工具的紧凑胶囊，收起时以宽度动画缩成圆形展开按钮。插入气泡包含相册、拍照和表格；排版展开三行 Keyline 图标格式检视器，分别选择段落层级、行内样式、列表缩进及对齐，正文按面板实际高度预留空间。焦点进入表格单元格时，右侧胶囊改为行列插入与删除动作。更多仅包含阅读、背景、导出和回收站。

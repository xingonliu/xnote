...
| F1 | Flutter 工程与质量门禁 | 代码完成（2026-09-04；六平台工程、CI/本地统一门禁及可用宿主构建均通过；按用户要求不再处理环境或设备启动验收） |
| F2 | Liquid Glass 与富文本阻断性 PoC | 代码完成（2026-09-04；Glass 与编辑器 PoC 代码及代码级测试已落地；按用户要求不执行模拟器或设备验收） |
| F3 | 纯 Dart 领域模型和规则 | 已完成（2026-09-04；纯 Dart 模型、编辑命令、历史、Markdown/检索/回收站规则与严格 JSON 已落地；静态分析及 55 项领域测试通过） |
| F4 | Drift 数据、FTS5、设置和文件层 | 代码完成（2026-09-04；schema version 1、事务仓储、FTS5、设置、搜索历史、附件文件与启动补扫已落地；静态分析及 19 项内存/文件数据库测试通过；按用户要求不执行模拟器或真机验收） |
| F5 | 主题、图标、Glass 应用壳和响应式导航 | 已完成（2026-09-04；语义主题、20 枚固定 Keyline SVG、直接 Glass 应用壳、go_router 独立分支栈及响应式导航已落地；静态分析、全量 101 项测试、手机/平板 Golden 与 debug APK 构建通过；按用户要求未启动模拟器） |
| F6 | 笔记本、笔记列表与基础编辑闭环 | 已完成（2026-09-04；真实 Drift 启动依赖、笔记范围/排序/最近编辑/多选、笔记本统计/手动排序/重命名/删除及 450 ms 自动保存编辑闭环已落地；静态分析、全量 104 项测试与手机/平板 Golden 通过；按用户要求未启动模拟器） |

...
- Bottom Tabs 的整条玻璃容器都是滑块手势区；横向拖拽越过触摸阈值后滑块以阻尼弹簧连续跟手，松手后吸附到最近的 tab；普通轻触仍由目标 tab 的可访问点击语义处理。
...
- 从 Bottom Tabs 内任意位置按下并横向拖动时，滑块越过触摸阈值后以阻尼弹簧连续跟手，松手后选中最近的 tab；普通点击仍能准确切换目标。

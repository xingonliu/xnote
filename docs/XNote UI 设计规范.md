...
>
> 视觉基准：Apple WWDC26 / Human Interface Guidelines / Liquid Glass
>
...
- Header、导航和悬浮工具使用公共 Liquid Glass 控件；正文及纸张保持稳定、不透明的内容层。普通按钮使用中性玻璃，主操作和选中态使用蓝色，危险操作使用红色。
- 仅笔记首页右下角新建「+」保留浅色 `#E09F3E` / 深色 `#FFD60A`、白色图标及既有尺寸、形状与动效，使用专属令牌；笔记本详情的新建按钮使用全局蓝色。
...
首页与笔记本详情长按进入多选时，底部操作栏使用公共玻璃 Panel。数量和取消在首行，移动与移入回收站操作在下一行按可用宽度自动换行；按钮文字保持单行。列表底部按操作栏实测高度、底部导航和安全区共同预留空间，末项滚动后不得被遮挡；平板首页操作栏同时避让左侧导航栏。
...
- 当前主题 `primary` 是笔记相关主强调色：浅色模式固定为 `#0066CC`，深色模式固定为 `#409CFF`；用于主操作、选中态和关键图标，不大面积铺满页面。
...
- Bottom Tabs 的选中图标和文字在浅色模式使用 `#0066CC`，在深色模式使用 `#409CFF`，且不存在组件私有强调色。

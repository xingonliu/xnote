...
>
> 视觉基准：iOS 备忘录 / Liquid Glass
>
> 关联文档：[XNote 功能清单与页面组成](./XNote%20功能清单与页面组成.md)、[XNote 开发顺序](./XNote%20开发顺序.md)、[XNote Agent 记忆与上下文架构](./XNote%20Agent%20记忆与上下文架构.md)
...
3. 所有页面统一接入基于 Backdrop 的 1 px 模糊与背景渐隐；二级页面顶部常驻渐进模糊遮罩，固定底部按钮或操作栏存在时才显示底部遮罩。
...
- 首页主体为笔记本宫格，卡片展示可选颜色的 Keyline 图标、名称、笔记数和最近编辑笔记的标题或摘要（最多两行）；卡片采用弹性底部布局，最新摘要置于平滑微容器中以保持同排等高时的视觉稳定，空笔记本显示记录想法的提示；无笔记本时统一由 `XNoteEmptyState` 承接。提供金、蓝、紫、绿、玫瑰色和笔记、星标、收集箱图标，浅色和深色主题分别适配。手机通常两列，窄窗口和大字号自动减少列数；同排卡片等高。
- “全部笔记”、“未分类”（副标题“灵感收集箱”）、“最近删除”收纳在一个 Inset Grouped 圆角容器中，使用缩进分隔线，每行有专属 Keyline 图标、专属色彩徽章底座、数字徽标和进入箭头。“我的笔记本”旁提供新建入口。
- 卡片与系统集合容器在浅色和深色主题下均通过低对比度描边（outlineVariant）建立清晰边界，避免浅色纯白背景下的层次混淆。
- 点击笔记本或系统集合进入笔记列表；长按笔记本触发一次触感反馈并在卡片旁展开公共 LiquidGlass 上下文菜单：重命名、更换颜色/图标、删除笔记本。删除对话框明确笔记本不可恢复，并默认选择将笔记移到未分类，也可选择连同笔记移入最近删除。
- 首页标题右上角提供快速管理入口，管理列表通过手柄拖拽排序，边缘自动滚动；无障碍服务提供上移/下移动作。完成或进入某本的更多菜单时保存顺序，取消放弃本次排序。平板栏共享卡片，浮层使用整个工作区，当前笔记本保留选中语义与容器色。
- 创建笔记本使用阻断式 `XNoteDialog` 输入名称，确认后进入该笔记本；在详情中新建自动归属当前笔记本，首页和系统集合中新建归入未归档。编辑返回原列表，再返回首页宫格。
- 普通按钮由公共组件提供中性前景，有色按钮由公共组件提供白色前景。图标与文字统一使用 `XNoteButtonContentSpacing = 8.dp`，左右内边距仍为 8 dp，图标按钮居中；业务组件不得自定义另一套图文间距。

## 2. 平台与技术边界

XNote 当前是 Android/Jetpack Compose 应用。页面边缘过渡使用 Backdrop 捕获层、1 px 模糊与主题底色渐隐；顶部和底部共享统一材质参数。

Android 实现统一封装为 `XNoteProgressiveBlur`，由 `XNotePageScaffold` 接入。它至少包含以下能力：

- `drawPlainBackdrop` 提供背景捕获，`blur(1f)` 使用固定 1 个物理像素的模糊半径，`runtimeShaderEffect` 实现主题底色渐隐。
- 渐进模糊使用独立捕获层，包含页面背景和正文，Header、Footer 与 Toast 在该层外绘制；编辑页 Header 和 Footer 的透明玻璃采样包含图片与正文的完整页面图层（边缘渐隐前），正文内玻璃控件继续采样背景层，避免循环采样。
- 二级页面顶部常驻，固定底部按钮或操作栏存在时底部也常驻；一级页面可以根据滚动状态显示需要的边缘。
- 顶部和底部只改变距离方向，共享同一组材质参数。
- 项目材质参数：覆盖高度 128 dp，模糊半径固定为 1 px（不随屏幕密度换算）；主题底色从正文端的透明平滑增强，在完整覆盖高度内连续过渡，仅在屏幕边缘完全覆盖；文字逐渐融入背景，浅色与深色共用同一遮色曲线。这些是近似 iOS 柔和滚动边缘的项目参数，不是 Apple 公布的系统数值。
...
| -------------- | --------------------------- | --------------------------------------------------- |
| 渐进模糊边缘   | iOS 27 页面边缘遮罩         | Backdrop 1 px 模糊 + 主题底色渐隐             |
...
- 所有二级页面必须常驻顶部边缘；仅有固定底部按钮或操作栏的二级页面同时常驻底部边缘。搜索、外观、普通回收站及其他无固定底部操作的页面不声明底部边缘。
...
- 声明渐变边缘的页面，首尾内容留白按 `max(128 dp 遮罩高度, 含系统栏的固定控件高度) + 16 dp` 计算；无底部遮罩时仅避让系统导航栏。列表、网格和编辑器将留白放在滚动内容内，滚动到两端时首项与末项均完全离开遮罩。阅读分页、导出预览和平板各栏使用同一规则；编辑器底部还需随工具栏实际高度变化。
- 编辑器滚动视口延伸至底部边缘；工具栏和导航栏的避让留白放在滚动内容内，确保正文在按钮后方连续渐隐，且末尾内容可滚动到工具栏上方。
- 边缘效果只负责层级过渡，不能额外占据布局高度，也不能遮挡标题、首行内容、滚动条或底部最后一项。
- 长列表、网格、笔记编辑器、阅读模式和对话消息区均遵循同一规则。

## 4. 二级页面 Header

### 4.1 标准结构

普通二级页面使用 `XNoteHeader`；编辑页使用同材质的 `EditorHeader`，中间留空，右侧提供撤销与更多。普通页面结构为：
...
笔记编辑页不在 Header 中间显示固定页面标题或保存状态。笔记标题属于文档内容，置于正文顶部并可编辑；标题下方以 `yyyy年M月d日` 显示最近一次成功保存的日期，新建笔记保存前显示创建日期。

编辑页 Header 在顶部安全区下保留 15 dp 留白，全部 40 dp 高按钮与笔记本选择器在同一行垂直居中；左右留白 16 dp，按钮组间距 8 dp，笔记本选择器占用剩余空间并限制最大宽度为 220 dp。Footer 在系统导航栏或键盘安全区上方额外保留 16 dp 留白，手机左右留白 16 dp、平板 24 dp。

编辑页底栏左侧是独立的圆形 Liquid Glass 插入按钮；右侧默认展开为紧凑胶囊，内含排版、待办、引用和收起工具四个图标，间距为 0。收起时胶囊以 180 ms 宽度动画向右收缩为圆形展开按钮，展开时同样向左长出其余图标；启用「减少动画」时直接显隐。插入按钮上方展示相册、相机、表格气泡菜单。排版展开 `XNoteRichTextToolbar` 三行格式检视器，全部使用 Keyline Stroke 图标：段落层级、行内样式、列表缩进与对齐。活跃格式仅将图标显示为 Primary 主题色，减少动画时直接显隐。面板实际高度计入正文底部留白。表格单元格获得焦点时，右侧胶囊切换为行上、行下、列左、列右、删行、删列、删表七个图标动作。Header 返回、撤销、更多和 Footer 左侧插入复用透明 `LiquidButton`；撤销为独立的 40 dp 圆形按钮，按历史状态启用。Header 历史操作与 Footer 右侧胶囊直接共用 `LiquidButton` 的材质和按压形变实现：vibrancy、2 dp blur、12/24 dp lens；父容器只承担玻璃渲染与整体动效，点击和无障碍语义由内部图标提供。Footer 右侧操作共用一个 Liquid Glass 胶囊，格式检视器共用一块玻璃面板；各图标保留独立点击区域，激活时仅图标变为 Primary 主题色，保留无障碍选中语义，禁用时降低图标不透明度。禁止用字符充当图标。

图片作为纸面自由贴纸显示：图片圆角 12 dp，选框外扩 6 dp；抗锯齿虚线跟随旋转。右上角是可撤销的删除玻璃按钮，右下角是固定触控尺寸的 Primary 复合手柄，以中心极角和距离同步控制旋转与 0.15–4 倍缩放。本体单指自由平移，保留双指缩放旋转，不限制到原图片行或正文宽度。选中图片上方 8 dp 展示替换、前移一层、后移一层、复制、重置胶囊。控制层与正文 Backdrop 捕获层为同级。每张图片按缩放与旋转后的外接高度独立参与文字排版，外接区域顶部对齐插入位置；缩小后下方正文随之上移。平移保持自由覆盖，变换包围盒继续扩展纸面可滚动范围。

编辑页 Header 左侧为返回，中间留空，右侧依次为撤销、更多。撤销按真实历史记录启用。笔记归属通过列表移动操作设置。更多只展示阅读模式、信纸背景、导出图片和移入回收站。
...
| Hero | 40 × 40 dp；空状态、错误状态主图标，以及 52 dp 新建笔记按钮内的加号 |
| 最小触控区域 | 40 × 40 dp |
...
全部笔记、未归档与笔记本详情长按进入多选时，底部操作栏使用公共玻璃 Panel。数量和取消在首行，移动与移入回收站操作在下一行按可用宽度自动换行；按钮文字保持单行。列表底部按操作栏实测高度、底部导航和安全区共同预留空间，末项滚动后不得被遮挡。
...
- 编辑页打开阻断式浮层前先收起输入法，手机底部抽屉不得被键盘压缩，系统返回一次即可关闭浮层。
...
- 长选择和筛选优先使用底部抽屉；图片来源使用插入入口上方气泡。
- 二级页面顶部常驻 Progressive blur；固定底部按钮、操作栏、底部导航和输入区存在时才声明底部边缘。
- Bottom Tabs 外层占位保持 88 dp，内部顶部 12 dp、底部 20 dp，玻璃容器高度为 56 dp，选中滑块静止高度为 48 dp。
- 浮动按钮不得遮挡列表最后一项，页面 Scaffold 负责补足底部内容间距。

### 10.2 平板

- 笔记区标准字号下在 840 dp 起使用多栏，1200 dp 起展开 Navigation Rail、笔记本、笔记列表、正文四栏；阈值按大于 1 的有效字体倍率提高。中等窗口编辑时收起笔记本栏，窄窗口保留单栏返回行为。
- 搜索框、历史与结果位于笔记列表栏；正文、独立纸张背景、Header 和编辑工具栏位于内容栏。各栏独立保留系统安全区与顶部渐进模糊；笔记列表的固定底部操作和编辑工具栏所在栏保留底部渐进模糊。当前笔记及笔记本以主题容器色标识，选择态提供无障碍语义。
...
- 页面由 `XNotePageScaffold` 承载；二级页面顶部常驻 AndroidLiquidGlass Progressive blur，有固定底部按钮或操作栏时底部也常驻，无底部操作时不显示底部遮罩。
...
- Markdown 快捷输入开启时只转换本次明确完成的语法，输入法组合态、未闭合标记、多行粘贴和无法无损表达的写法均保留原文；关闭后不发生任何自动格式化。
- 自动格式化后的第一次撤销恢复原始 Markdown 字符，光标与选择区停留在可继续输入的位置。
- 编辑页在键盘显示时打开背景、移动、删除或链接浮层，浮层使用完整安全区且一次系统返回即可关闭。
- 编辑页可通过公共背景选择器修改当前单篇笔记背景，设置页可修改默认背景，并准确展示各自影响范围。
- 笔记编辑页只存在一层全屏笔记背景，系统栏、Header 和底部工具区周围不得露出第二层页面底色。
...
以下为待实现的 S11 交互约束，完整范围见 [S11 实施方案](./XNote%20S11%20实施方案.md)。

- 对话默认发送为补充当前任务，另提供加入队列；队列可编辑、删除和排序。停止和中断状态明确显示，未完成回复不伪装为正常结束。
- 顶部显示当前模型，不提供切换入口；可快速访问三级权限和数据范围；初始一级不可查看、范围仅主动附加。笔记卡片注明发送版本和临时读取范围。
- 实时改动按单篇显示累计 Diff，每篇提供全部接受、全部拒绝；创建和删除为整篇卡片，不提供逐处接受/拒绝。
- 拒绝发生冲突时整次暂停，说明用户编辑已保留，并提供保留当前内容或重新调整入口。
- 待审阅状态跨页面和重启保留；接受是确认已应用内容，不能使用“接受后首次保存”的文案。
- 自动记忆开关说明仅控制画像自动新增和更新，不关闭历史保存、上下文压缩或已有记忆使用。
- 图片、PDF、纯文本和 Markdown 附件在发送前检查模型能力；不支持或超限时保留草稿并显示处理入口。
- 后台通知展示运行状态与停止入口；进程终止后显示中断和继续入口，不自行恢复写操作。
- 手机与平板均复用公共页面、玻璃控件和状态组件；具体布局随 S11 对应子切片验收。

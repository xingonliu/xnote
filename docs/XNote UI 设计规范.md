...
3. 所有页面统一接入基于 Backdrop 的 1 px 模糊与背景渐隐；二级页面顶部和底部必须常驻连续的渐进模糊遮罩。
...
- 搜索框、历史与结果位于笔记列表栏；正文、独立纸张背景、Header 和编辑工具栏位于内容栏。每个内容栏独立保留系统安全区和渐进模糊。当前笔记及笔记本以主题容器色标识，选择态提供无障碍语义。

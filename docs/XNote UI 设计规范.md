...
- Bottom Tabs 选中胶囊的几何中心必须始终与当前 tab 的图标和文字中心重合；按压下陷、拖拽形变和色散不得改变中心锚点或产生累计偏移。
- Bottom Tabs 悬停或取得焦点时，目标 tab 显示轻微折射、低密度主题色光晕和前景软化，作为不改变选中项的预激活反馈。
- Bottom Tabs 按压时，受力位置的中性基础内容层与主题色内容层共享向下位移与轴向压缩，滑块同时缩短并增强内阴影，不得让灰色图标或文字残留在原位。
- Bottom Tabs 的整条玻璃容器都是滑块手势区；横向拖拽越过触摸阈值后滑块连续跟手，松手后吸附到最近的 tab；普通轻触仍由目标 tab 的可访问点击语义处理。
- 拖拽中的主题色内容层必须只通过滑块和液桥实体的几何范围曝光。滑块边缘横切图标或文字时，覆盖部分高亮、外露部分保持低透明度线框，交界必须沿同一个连续曲线路径进行亚像素裁切。
- 滑块离开当前 tab 后保留逐渐收窄的液桥与源端残影；位移达到相邻 tab 中位阈值时液桥快速断开，释放后使用欠阻尼弹簧吸附并输出一次遵循系统设置的确认触觉。
...
- 按下时使用轻微的纵向下陷、轴向压缩、聚光和内阴影变化，不能造成组件布局位置跳动。
...
- Bottom Tabs 在静止、按压下陷和拖拽状态下，选中胶囊均以当前 tab 中心为锚点，不向任一侧产生非交互预期的偏移。
- Bottom Tabs 按压时，选中项的中性基础图标与文字不会残留在原位置，且另外两个未选中项不会随之缩放。
- 从 Bottom Tabs 内任意位置按下并横向拖动时，滑块越过触摸阈值后连续跟手，松手后选中最近的 tab；普通点击仍能准确切换目标。
- 点击另一个 tab 时，滑块先唤起并从当前位置动画移动到目标，页面切换不得绕过或吞掉该滑块反馈。
- 悬停、D-pad 或键盘焦点能够触发目标 tab 的预激活折射与微光晕，且不会提前切换页面。
- 滑块横切未激活图标时，只有滑块或液桥实际覆盖的路径变为主题色，图标外露部分保持中性线框，裁切边缘无整图闪变或矩形穿帮。
- 拖拽未过中位阈值时液桥仍连接源 tab；越过阈值后液桥断开，释放吸附包含短促欠阻尼微震并仅输出一次确认触觉。

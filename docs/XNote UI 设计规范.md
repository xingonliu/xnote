...
- 首页主体为笔记本宫格，卡片展示可选颜色的 Keyline 图标、名称、笔记数和最近编辑笔记的标题或摘要（最多两行）；空笔记本显示记录想法的提示。提供金、蓝、紫、绿、玫瑰色和笔记、星标、收集箱图标，浅色和深色主题分别适配。手机通常两列，窄窗口和大字号自动减少列数；同排卡片等高。
- “全部笔记”、“未分类”（副标题“灵感收集箱”）、“最近删除”收纳在一个 Inset Grouped 圆角容器中，使用缩进分隔线，每行有专属 Keyline 图标、数字徽标和进入箭头。“我的笔记本”旁提供新建入口。
